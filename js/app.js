...
const App = (() => {
  ...
  function setupSearch() {
    ...
    if (!box) {
      ...
      box.style.cssText =
        'position:absolute;z-index:99;background:#1a222d;border:1px solid #2a3441;' +
        'border-radius:8px;max-height:280px;overflow:auto;display:none;width:min(400px,92vw);left:0;top:100%';
      input.parentElement.style.position = 'relative';
      input.parentElement.appendChild(box);
    }
    input.addEventListener('input', () => {
      const q = input.value.trim().toLowerCase();
      if (!q || equityIndex.length === 0) {
        box.style.display = 'none';
        return;
      }
      const hits = [];
      for (let i = 0; i < equityIndex.length && hits.length < 15; i++) {
        const x = equityIndex[i];
        const sym = (x.symbol || '').toLowerCase();
        const name = (x.name || '').toLowerCase();
        if (sym.indexOf(q) === 0 || sym.indexOf(q) >= 0 || name.indexOf(q) >= 0) hits.push(x);
      }
      if (!hits.length) {
        box.style.display = 'none';
        return;
      ...
      box.innerHTML = hits
        .map(
          (h) =>
            '<div class="sug-item" data-sym="' +
            h.symbol +
            '" style="padding:9px 12px;cursor:pointer;font-size:13px;border-bottom:1px solid #2a3441">' +
            '<b>' +
            h.symbol +
            '</b> <span style="color:#8b9aab">' +
            (h.name || '') +
            '</span></div>'
        )
        .join('');
      ...
      box.querySelectorAll('.sug-item').forEach((el) => {
        el.onmouseenter = () => (el.style.background = '#243044');
