...
const App = (() => {
  ...
  function renderMarketView() {
    ...
    $('#m-price').textContent = formatINR(last.close);
    $('#m-delta').textContent = `${change >= 0 ? '+' : ''}${change.toFixed(2)} (${pct >= 0 ? '+' : ''}${pct.toFixed(2)}%)`;
    $('#m-delta').className = `delta ${change >= 0 ? 'positive' : 'negative'}`;
  ...
  function renderValuationWidgets(last) {
    ...
    // Always refresh defaults from Yahoo when ticker changes
    const epsVal = info.trailingEps != null && info.trailingEps > 0 ? info.trailingEps : 50;
    const bvVal = info.bookValue != null && info.bookValue > 0 ? info.bookValue : 400;
    ...
    // Simple multi-stage DCF
    const fcfEl = $('#dcf-fcf');
    const sharesEl = $('#dcf-shares');
    const gEl = $('#dcf-growth');
    const dEl = $('#dcf-discount');
    // Prefill DCF from Yahoo (FCF & shares in crores)
    let fcfCr = 15000;
    if (info.freeCashflow != null && info.freeCashflow !== 0) {
      fcfCr = info.freeCashflow / 1e7; // Yahoo is in absolute currency units
    }
    let shCr = 650;
